  ...
  try {
    ...
    if (roomExist && Math.round(number/roomExist.capacity )< 100) {
      ...
      if (newTransaction) {
        ...
        const participationMail = participation(username,name,montant,prix,category,room)
...
const deleteParticipation = async (req, res) => {
  ...
  const numberOfDocs = await Transaction.aggregate([ // User is the model of userSchema
  {
    $group: {
