...
const addOrder = async (req, res) => {
  try {
    ...
    const newOrder = await Order.create({
      ref_order: `${NumberOfOrders + 1}${name
        .substring(0, 2)
        .toUpperCase()}${(
...
const getOrder = async (req, res) => {
  try {
    ...
    Order.paginate( {}, options, function (err, result) {
      ...
      res.json({orders:result.docs,total:result.totalDocs,totalPages:result.totalPages,page:result.page});
      // result.docs
...
const getOrderByRoom = async (req, res) => {
  try {
    ...
    const singleOrder = await Order.findOne({room_id:req.params.id,user_id:req.params.user})
      .populate("user_id")
      .populate({
        path: "room_id",
        populate: {
...
const validationMail = async (req,res) => {
  const {email ,ref, method_of_paiement , image , status_payment , name , prix ,prixPromo,order_id} = req.body
  const validation = orderValidation(ref , name, prixPromo ,method_of_paiement,status_payment,order_id)
  var transporter = nodemailer.createTransport({
    service: "gmail",
    auth: {
      user: "boubakermeriem62@gmail.com",
...
const getNumberOfDocumentsByMonth = async(req,res) =>{
  const numberOfDocs = await Order.aggregate([ // User is the model of userSchema
  {
    $group: {
