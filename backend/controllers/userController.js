...
const userRegister = async (req, res, next) => {
  try {
    ...
    const newUser = await User.create({
      ...
      isAdmin,
      genre,
      profile_img:"631207d96999d98117b7de58",
...
const imageUpload = async (req, res) => {
  try {
    ...
    if(newimage){const updateduser = await User.findByIdAndUpdate(existuser._id, {
...
const addCredit = async (req, res) => {
  try {
    ...
    if (newTransaction) {
      ...
      res.send({ status: "COMPLETED", montant: newCredit.credit.montant });
      const userInfo = await User.findById(user);
      const creditMail = credit(newCredit?.username,montant,newCredit?.credit?.updatedAt);
      var transporter = nodemailer.createTransport({
        service: "gmail",
        auth: {
          user: "boubakermeriem62@gmail.com",
  ...
  try {
    ...
    if (!user) {
      ...
      if (
        ...
        user?.profile_img?._id?.toString() !== "6375ebb216a3ddb2437df262"
      ) {
